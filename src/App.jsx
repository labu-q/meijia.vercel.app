...
function Team({ data }) {
  ...
  return <section className="section page-section"><h1>技师业绩</h1><p className="subtitle">本月服务收入与预估提成</p>{scores.length ? <div className="team-list">{scores.map((item) => <article className="technician" key={item.id}><div className="avatar purple">{item.name[0]}</div><div className="tech-main"><b>{item.name}</b><small>完成 {item.orders} 单 · 提成 {(item.commission * 100).toFixed(0)}%</small><div className="bar"><i style={{ width: `${Math.min(100, item.sales / 8)}%` }} /></div></div><div className="tech-money"><b>{money(item.sales)}</b><small>提成 {money(item.bonus)}</small></div></article>)}</div> : <EmptyState icon="♧" title="还没有技师" text="添加技师后，即可在这里查看服务业绩。" />}</section>;
}
...
function Mine({ data }) {
  ...
  return <section className="mine-page"><div className="shop-card"><div className="shop-logo">甲</div><div><small>我的店铺</small><h1>小鹿美甲工作室</h1><p>经营数据已安全保存到本机</p></div><span>›</span></div><div className="mine-stats"><article><b>{data.customers.length}</b><span>客户</span></article><article><b>{data.technicians.length}</b><span>技师</span></article><article><b>{money(totalIncome)}</b><span>累计营收</span></article></div><div className="setting-list"><button><i>▣</i><span>店铺资料</span><b>›</b></button><button><i>◫</i><span>收支分类管理</span><b>›</b></button><button><i>⇩</i><span>数据备份与导出</span><b>›</b></button><button><i>?</i><span>使用帮助</span><b>›</b></button></div><p className="version">甲·账本 v1.0 · 数据仅保存在当前设备</p></section>;
}

export default function App() {
  const [data, setData] = useState(loadData); const [page, setPage] = useState('home'); const [modal, setModal] = useState(null);
  useEffect(() => saveData(data), [data]);
  const content = page === 'home' ? <Dashboard data={data} openForm={setModal} /> : page === 'book' ? <Bookkeeping data={data} setData={setData} openForm={setModal} /> : page === 'flow' ? <Flow data={data} setData={setData} /> : page === 'customers' ? <Customers data={data} setData={setData} /> : page === 'team' ? <Team data={data} /> : <Mine data={data} />;
